...
- 티커는 서버에서 정규화한다. 1~6자리 숫자 티커는 6자리로 zero-padding하고, 숫자를 포함한 6자리 영숫자 코드는 한국 종목 후보로 처리한다.
...
- 1~6자리 숫자 티커는 6자리 한국 종목 코드로 정규화하고, 숫자를 포함한 6자리 영숫자 코드는 한국 종목 후보로 처리한다.
...
- CLI 실행기는 Claude Code, Codex CLI, `agy`이며 분석 결과의 저장 모델 값은 각각 `claude-code`, `codex-cli`, `gemini-cli`이다. 각 실행기는 결과를 `analysis.md` 파일에 저장하도록 프롬프트를 받는다.
