...
def fetch_daily_df(ticker: str, start: date):
...
def run_evaluate_outcomes(db: Session, force: bool = False) -> dict[str, int]:
    ...
    if not force:
        stmt = stmt.where(Analysis.outcome.is_(None))
    ...
    def _fetch(ticker: str, earliest: date):
        try:
            df = fetch_daily_df(ticker, earliest)
        except Exception:
            return ticker, None
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {
            executor.submit(_fetch, ticker, earliest): ticker
