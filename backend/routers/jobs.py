...
매수 신뢰성 필터:
  ...
  지지 확인 시 매수, 돌파 시 매수처럼 조건이 아직 충족되지 않은 경우 최종 판정은 매수가 아니라 홀드.
  구름 위 + MA 정배열만으로는 매수 불가. MACD hist 증가, RSI 상승, volume_ratio_20 >= 1.0,
  현재가의 전환선/기준선 회복, 미래 구름 안정 중 최소 2개 이상이 확인되어야 매수 가능.
...
3. 구름 위 + MA 정배열 → 매수 신뢰성 필터를 통과할 때만 매수. 필터 미충족 또는 리스크 2개 이상이면 홀드.
