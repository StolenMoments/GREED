...
매수 신뢰성 필터:
  ...
  구름 위 + MA 정배열은 매수 판정의 주요 긍정 근거로 인정한다.
  지지권 근처 선제 진입 또는 저항 돌파 예상 진입도 손절 기준과 목표 저항이 명확하면 매수 판정 가능하다.
  MACD hist 증가, RSI 상승, volume_ratio_20 >= 1.0,
  현재가의 전환선/기준선 회복, 미래 구름 안정은 매수 신뢰도를 높이는 보조 근거로 사용한다.
...
3. 구름 위 + MA 정배열 → 손절 기준과 목표 저항이 명확하면 매수를 우선 검토. 리스크가 2개 이상이면 홀드 이하로 낮춤.
