...
매수 신뢰성 필터:
  매수 판정은 현재가 기준으로 즉시 또는 다음 거래일에 실행 가능한 우위가 있을 때만 사용.
  기본 구조 조건은 구름 위 + MA 정배열, 또는 명확한 돌파 직후/눌림 구조다.
  MACD hist 양전환 또는 증가만으로는 매수 판정 금지. 반드시 추세/구름/MA 구조와 함께 해석한다.
  매수 판정에는 다음 확인 신호가 2개 이상 필요하다:
  MACD hist 개선, RSI 상승 또는 45~65 회복 구간, volume_ratio_20 >= 1.0,
  현재가가 전환선/기준선 위, bearish divergence 없음.
  지지권 근처 선제 진입 또는 저항 돌파 예상 진입도 손절 기준과 목표 저항이 명확하고 확인 신호가 2개 이상이면 매수 판정 가능하다.
  강한 리스크가 1개 이상이면 매수 판정을 금지하고 홀드 이하로 낮춤:
  구름 안/아래, MA 역배열, 현재가가 기준선 아래, bearish divergence 또는 strict_divergence=bearish,
  MACD hist가 음수권에서 2주 이상 연속 감소.
  일반 리스크가 3개 이상이면 매수 판정을 금지하고 홀드 이하로 낮춤:
  RSI 2주 이상 하락, volume_ratio_20 < 0.7, 현재가가 전환선 아래,
  미래 구름 두께 축소 또는 하락운 전환 예정, MACD hist 둔화.
  구름 위 + MA 정배열 + 확인 신호 2개 이상 + 손절/목표 저항이 명확하면 일반 리스크 2개까지는 매수 검토 가능.

구름 위치별 기본 판정:
  구름 위: 상승 지지 구조로 보되, 모멘텀 증가와 거래량 확인이 부족하면 홀드.
  구름 안: 기본 판정은 홀드. 구름 상단을 종가 기준 돌파하기 전까지 매수 금지.
  구름 아래: 기본 판정은 매도 또는 홀드. 반등 가능성은 조건부 시나리오로만 작성.

손절/목표 신뢰성:
  시스템 평가는 저가 터치에 민감하므로 손절선이 현재가 대비 과도하게 타이트하지 않은지 검증.
  atr14_pct >= 6%이면 단순 지지선 바로 아래 손절을 피하고 변동성 노이즈를 감안.
  손절 기준은 저가 터치 기준인지 종가 기준인지 조건 문장에 명시.
  목표가는 가장 가까운 실제 저항을 우선 사용하고, 돌파가 필요한 상위 가격은 중기 목표로만 언급.

NaN 처리: NaN 구간 지표는 판단에서 제외하고 명시.

재무 보조 지표 해석 (프롬프트 하단 '재무 지표' 블록이 제공된 경우에만 사용):
  PER/PBR: 동일한 기술적 신호에서 고PER/고PBR이면 목표가를 보수적으로 잡고, 저PER/저PBR이면 보유/매수 신뢰를 보강하는 보조 근거로만 사용.
  배당수익률: 배당수익률이 높으면 하방 지지 보조 근거로 참고.
  시가총액: 소형주는 변동성·유동성 리스크를 감안.
  재무 추이(분기 표): EPS/BPS가 추세적으로 증가하면 실적 개선, 감소하면 실적 둔화로 해석하고 차트 추세와 같은 방향이면 신뢰를 보강, 반대면 주의 근거로 사용.
  밸류에이션 밴드: 현재 PER/PBR이 3년 밴드의 저평가 영역(하위 구간)이면 가격 매력 보조 근거, 고평가 영역(상위 구간)이면 목표가를 보수적으로 잡고 추격 매수에 주의.
  재무 지표는 보조 참고용이며 기술적 판정(매수/홀드/매도)을 단독으로 뒤집지 않는다.
  값이 N/A이거나 '조회 실패'이거나 블록이 제공되지 않으면 재무는 제외하고 기술적 분석만 수행한다.
...
- `근거`에는 구조 근거 1개와 확인 신호 2개 이상을 실제 수치로 쓰세요.
...
매매 판정 우선 규칙 (번호 순서대로 적용):
1. 구름 아래 + MA 역배열 → 매도. 다른 지표가 긍정적이어도 이 판정을 유지.
2. 구름 안(가격이 구름 안에 위치) → 홀드. 돌파/반등 매수는 조건부 시나리오로만 작성.
3. 구름 위 + MA 정배열이라도 확인 신호 2개 미만이면 홀드. 확인 신호 2개 이상이고 손절 기준과 목표 저항이 명확하면 매수를 우선 검토.
4. 강한 리스크가 1개 이상이거나 일반 리스크가 3개 이상이면 확인 신호가 있더라도 매수하지 않고 홀드 이하로 낮춤. 구름 위 + MA 정배열 + 확인 신호 2개 이상 + 손절/목표 저항이 명확하면 일반 리스크 2개까지는 매수 검토 가능.
...
뉴스·경제 이슈 등 외부 정보나 프롬프트에 제공되지 않은 재무 수치를 임의로 언급 금지.
단, 하단에 '재무 지표' 블록이 제공된 경우 해당 수치는 보조 근거로 활용 가능.
...
def list_job_overview_endpoint(
    ...
    strategy_stmt = select(BacktestStrategyJob).order_by(BacktestStrategyJob.id.desc())
    if allowed_statuses:
        strategy_stmt = strategy_stmt.where(BacktestStrategyJob.status.in_(allowed_statuses))

    for job in db.scalars(strategy_stmt).all():
        rows.append(
            JobOverviewRead(
...
_FUNDAMENTALS_UNAVAILABLE_BLOCK = (
    "재무 지표 (분석 보조): 조회 실패 — 재무 요소는 제외하고 기술적 분석만 수행하세요."
)
...
def _build_fundamentals_block(db: Session, ticker: str, market: str | None = None) -> str:
    ...
    block = _format_fundamentals_block(snapshot)
    try:
...
def _format_fundamentals_block(snapshot: FundamentalSnapshot) -> str:
    ...
    def market_cap(value: float | None) -> str:
        if value is None or value <= 0:
            return "N/A"
        return f"{value / 1e8:,.0f}억원"

    date_str = snapshot.snapshot_date.isoformat() if snapshot.snapshot_date else "확인 불가"
    return (
        f"재무 지표 (분석 보조 · 스냅샷일자 {date_str}):\n"
        f"- PER: {ratio(snapshot.per)}\n"
        f"- PBR: {ratio(snapshot.pbr)}\n"
        f"- EPS: {amount(snapshot.eps, '원')}\n"
        f"- BPS: {amount(snapshot.bps, '원')}\n"
        f"- 배당수익률: {percent(snapshot.div_yield)}\n"
        f"- 시가총액: {market_cap(snapshot.market_cap)}"
    )


def _quarter_label(snapshot_date: date) -> str:
    return f"{snapshot_date.year}Q{(snapshot_date.month - 1) // 3 + 1}"


def _format_fundamentals_trend(
    history: list[FundamentalHistory],
    snapshot: FundamentalSnapshot,
) -> str:
    """Quarterly trend table + PER/PBR valuation band from the history series."""
    if not history:
        return ""

    rows = list(history)
    quarter_rows = [r for r in rows if r.snapshot_date.month in (3, 6, 9, 12)]
    if len(quarter_rows) < 2:
        quarter_rows = rows
    quarter_rows = quarter_rows[-12:]
    if not quarter_rows:
        return ""
    ...
    lines = ["재무 추이 (분기말, 최근 3년):", "분기 | PER | PBR | EPS | BPS"]
    for row in quarter_rows:
        lines.append(
            f"{_quarter_label(row.snapshot_date)} | {ratio(row.per)} | {ratio(row.pbr)} | "
            f"{amount(row.eps)} | {amount(row.bps)}"
        )

    per_band = valuation_band([r.per for r in rows], current=snapshot.per)
    pbr_band = valuation_band([r.pbr for r in rows], current=snapshot.pbr)
    band_lines = ["밸류에이션 밴드 (최근 3년 기준):", _band_line("PER", per_band), _band_line("PBR", pbr_band)]

    return "\n".join(lines + [""] + band_lines)


def _band_line(label: str, band: dict[str, float] | None) -> str:
    if band is None:
        return f"- {label}: N/A"
    ...
    if percentile < 33:
        ...
        zone = "중립 구간"
    return (
        f"- {label}: 현재 {band['current']:,.2f} · 3년 {band['min']:,.2f}~{band['max']:,.2f} "
        f"(중앙값 {band['median']:,.2f}) · 하위 {percentile:.0f}% ({zone})"
    )
