...
매수 신뢰성 필터:
  ...
  다음 리스크가 2개 이상이면 매수 판정을 금지하고 홀드 이하로 낮춤:
  MACD hist 2주 이상 연속 감소, RSI 2주 이상 하락, volume_ratio_20 < 0.7,
  현재가가 전환선 또는 기준선 아래, 미래 구름 두께 축소 또는 하락운 전환 예정,
  bearish divergence 또는 strict_divergence=bearish.
...
4. 리스크가 2개 이상이면 확인 신호가 있더라도 매수하지 않고 홀드 이하로 낮춤.
