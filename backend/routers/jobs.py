...
매매 판정 우선 규칙 (번호 순서대로 적용, 조건 충족 시 강제 적용):
1. 구름 아래 + MA 역배열 → 매도. 다른 지표가 긍정적이어도 이 판정을 유지.
2. 구름 위 + MA 정배열 + RSI < 70 → 매수를 우선 판정. 단, 명확한 하락 반전 신호가 있으면 홀드로 조정 가능.
3. 구름 안(가격이 구름 안에 위치) → 홀드를 기본 판정으로 시작. 아래 조건이 충족될 때만 변경:
   - MACD bullish cross 발생 + RSI가 직전 2주 대비 상승 중 → 매수로 변경 가능
   - RSI > 75 + MACD bearish cross 발생 → 매도로 변경 가능
